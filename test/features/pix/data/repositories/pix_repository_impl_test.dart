...
import 'package:rppay/features/pix/data/repositories/pix_repository_impl.dart';
...
void main() {
  group('PixRepositoryImpl', () {
    ...
    test('a primeira ação é "Transferir" com o tipo correto', () async {
      final actions = await repository.getPixActions();
      final first = actions.first;

      expect(first.title, 'Transferir');
      expect(first.subtitle, 'Pague com chave Pix');
      expect(first.type, PixActionType.transfer);
    });
  });
}
